...
export class RejectedComponent implements OnInit {
  ...
  positions: any[] = [];
  selectedCard: RejectedCard | null = null;
  showEditModal = false;
  ...
  fetchRejectedCards(): void {
    this.loading = true;
    this.error = '';
    const filters = this.filterData || {};
    //console.log('Fetching rejected cards with filters:', filters);

    this.authService.getRejectedCards(filters).subscribe({
      next: (response) => {
        //console.log("Received rejected cards data:", response);
        if (response && response.data) {
          this.rejectedCards = response.data;
          //console.log("verified approved cards reamrsks data " , this.rejectedCards[0]?.verified_approveds );

        } else {
          this.rejectedCards = Array.isArray(response) ? response : [];
        }
        //console.log("Assigned rejectedCards:", this.rejectedCards);
        this.loading = false;
      },
      error: (err) => {
        this.error = 'Failed to load rejected applications. Please try again.';
        this.loading = false;
        console.error('Error fetching rejected cards:', err);
      }
    });
  }
  ...
  // Helper method to get full image URL - SAME AS APPLIED CARDS
  ...
  openEditModal(card: RejectedCard): void {
    ...
    // Set current file URLs from the card data
    ...
    // Set the designation based on current position
  ...
  // UPDATED viewImage method - same as applied cards
  ...
  // Handle image load errors with debugging - SAME AS APPLIED CARDS
  ...
  // Debug method to test URL accessibility - SAME AS APPLIED CARDS
    ...
    this.authService.deleteFile(this.fileToDelete).subscribe({
      next: (response) => {
        ...
        if (this.selectedCard) {
          // Clear the URL from the card object
          (this.selectedCard as any)[this.fileFieldToDelete] = '';

          // Clear the corresponding URL property
          this.clearFileUrlByField(this.fileFieldToDelete);

          // Set the corresponding replace flag to true to show the file input
  ...
  /**
   * Simple file upload method for all file types - Same as registration component
   */
  UploadFile(event: any, operation: string) {
    ...
    if (file) {
      ...
      if (operation === 'photo') {
        // Photo field only allows JPEG/JPG
      ...
      this.authService.uploadFilePhoto(formData).subscribe({
        next: (res) => {
          // Set the URL based on operation type - UPDATED TO USE getFullImageUrl
          switch (operation) {
            case 'photo':
          ...
          this.toggleReplaceFile(operation, false); // Hide file input after success
        },
        error: (err) => {
  ...
  // Set loading state for specific file field - Same as registration
  ...
  submitUpdate(): void {
    ...
    this.authService.updateApplication(updateData).subscribe({
      next: (response) => {
      ...
      error: (err) => {
        ...
        this.error = message;
        console.error('Error updating rejected card:', err);
        alert(`Update failed: ${message}`);
      }
    });
  }

  // Modified to accept an optional state to force true or false
  ...
  // Fixed getVendorPositions method
  getVendorPositions(): void {
    this.authService.getvendorposition().subscribe({
      next: (res) => {
        //console.log('Position data received:', res);
      ...
      complete: () => {
        //console.log('Position fetch complete. Total positions:', this.positions.length);
      }
    });
  }

  // Fixed onPositionChange method
  onPositionChange(event: Event): void {
    const target = event.target as HTMLSelectElement;
    const selectedPositionName = target.value;

    //console.log('Position selected:', selectedPositionName);

    if (selectedPositionName) {
      // Find the position object from positions array
      const selectedPosition = this.positions.find(pos =>
        pos.vendorPositionName === selectedPositionName
      );

      if (selectedPosition) {
        // Update the designation property
        this.designation = selectedPosition.vendorPositionName;

        // Update both position and designation in the form
        this.updateForm.patchValue({
          position: selectedPositionName,
          designation: this.designation
        });

        //console.log('Updated designation:', this.designation);
      } else {
        console.warn('Position not found in positions array:', selectedPositionName);
        // Still update the form but with empty designation
        this.designation = selectedPositionName; // Use the selected value as designation
        this.updateForm.patchValue({
          position: selectedPositionName,
          designation: this.designation
        });
      ...
      // Clear both position and designation if nothing selected
      this.designation = '';
      this.updateForm.patchValue({
        position: '',
        designation: ''
      });
      //console.log('Position cleared, designation cleared');
    }
  }
}