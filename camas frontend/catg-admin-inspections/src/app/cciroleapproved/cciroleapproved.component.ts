...
@Component({
  selector: 'app-cciroleapproved',
  imports: [FormsModule, CommonModule,PdfViewerModule],
  templateUrl: './cciroleapproved.component.html',
  styleUrl: './cciroleapproved.component.css'
})
export class CciroleapprovedComponent {
  searchText: string = '';
  ...
  constructor(private apicall: ApicallService, private encservice: CryptoService, private modalService: NgbModal) { }
  ...
  getApproverIdcardData() {
    ...
    this.apicall.getcciapprovaldata(data).subscribe({
      next: (response: any) => {
       // console.log(response);
        if (Array.isArray(response.data)) {
          this.idcardsList = response.data;
